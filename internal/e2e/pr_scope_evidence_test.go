...
// TestPRFinalScopeKeepsForkTestingSummaryWithoutStaleEvidence reproduces the PR
// 1272 failure at the closest supported user-visible boundary: a real gate push
// executes the full pipeline and a GitHub PR creation receives the final body on
// stdin.
//
...
//     delta; this fork keeps the Test summary in the PR body, but the earlier
//     detailed evidence is step-scoped and never final PR scope.
...
//   - Earliest divergence from the proven accurate path: the pre-Document
//     detailed Test target is presented in the later PR body instead of
//     remaining evidence for that completed step.
...
func TestPRFinalScopeKeepsForkTestingSummaryWithoutStaleEvidence(t *testing.T) {
	...
	if !strings.Contains(body, "## Testing") {
		t.Fatalf("final PR body must keep this fork's Testing summary section:\n%s", body)
	}
	if !strings.Contains(body, "Focused validation passed at the test step target commit.") {
		t.Fatalf("final PR body missing the Test summary:\n%s", body)
	}
