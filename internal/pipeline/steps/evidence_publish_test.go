...
func TestPublishRunEvidence_LandsOnOrphanBranchAndLinksFromThePRBody(t *testing.T) {
	...
	wantImage := "https://raw.githubusercontent.com/example/widgets/" + tip + "/.no-mistakes/evidence/feature/add-login/checkout.png"
	if !strings.Contains(md, "![Checkout screenshot]("+wantImage+")") {
		t.Fatalf("expected the PR body to render the published screenshot from the evidence commit, got:\n%s", md)
	}
...
func TestPublishRunEvidence_PercentEncodesEveryArtifactPathSegment(t *testing.T) {
	...
	md := BuildTestingSummaryForPR(steps, rounds, sctx.Repo.UpstreamURL, sctx.Run.HeadSHA, sctx.WorkDir, links)
	want := "https://raw.githubusercontent.com/example/widgets/" + tip + "/evidence%20archive%20%231%20100%25/feature/add-login/capture%20%231%20100%25/checkout%20100%25.png"
