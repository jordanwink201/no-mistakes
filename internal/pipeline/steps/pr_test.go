...
func TestPRStep_AppendsTestingSectionFromTestStep(t *testing.T) {
	...
	for _, stale := range []string{"## Risk Assessment", "touches critical error handling"} {
		if strings.Contains(ghLog, stale) {
			t.Fatalf("final PR must not append earlier Review rationale %q, got:\n%s", stale, ghLog)
		}
	}
	if !strings.Contains(ghLog, "## Testing") {
		t.Fatalf("expected final PR to include the Testing evidence section, got:\n%s", ghLog)
	}
	if !strings.Contains(ghLog, "- 🔧 **Test** - 1 issue found → auto-fixed ✅") {
		t.Fatalf("expected final PR to summarize the Test evidence, got:\n%s", ghLog)
	}
	if !strings.Contains(ghLog, "<summary>🔧 **Test** - 1 issue found → auto-fixed ✅</summary>") {
		t.Fatalf("final PR must retain the compact Test step status, got:\n%s", ghLog)
	}
}
...
func TestPRStep_CreateKeepsGeneratedSectionsAfterOversizedIntent(t *testing.T) {
	...
	for _, unwanted := range []string{"## Risk Assessment"} {
		if strings.Contains(body, unwanted) {
			t.Fatalf("expected created PR body to exclude step-scoped Review rationale %q, got:\n%s", unwanted, body)
		}
	}
}
