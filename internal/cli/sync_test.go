...
func containsTOONScalar(out, key, value string) bool {
	return strings.Contains(out, key+": "+value) || strings.Contains(out, key+": "+fmt.Sprintf("%q", value))
}
...
func TestAxiSurfacesReportUserOwnedReleaseAfterUnmovedPrePushAbort(t *testing.T) {
	...
	for _, key := range []string{"head", "submitted_head", "current_head"} {
		if !containsTOONScalar(status, key, f.submitted) {
			t.Errorf("status missing %s for %q:\n%s", key, f.submitted, status)
		}
	}
