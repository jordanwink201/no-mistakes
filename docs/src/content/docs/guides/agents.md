...
That directory is always temporary and outside the worktree; GitHub visual evidence is hosted in secret gists by default, and repos can opt into publishing the directory to an orphan evidence branch with `test.evidence.store_in_repo`.
