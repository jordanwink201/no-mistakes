...
- The step records the exact tests and checks it exercised in a `tested` array, may include a short natural-language `testing_summary`, and includes an `artifacts` array for reviewer-visible evidence; `path` artifacts may be repository-relative paths, absolute paths under the temporary `no-mistakes-evidence/<runID>` directory, or managed evidence-directory paths eligible for hosted upload; `url` artifacts must be externally visible, and `content` artifacts should be short logs or command output shown directly in the PR.
- By default, evidence is stored under the temporary `no-mistakes-evidence/<runID>` directory, and GitHub visual evidence is uploaded to secret gists while the PR body is assembled. With `test.evidence.store_in_repo: true`, evidence is stored under `<test.evidence.dir>/<branch-slug>` inside the worktree, staged during push, and published with the branch. Unsafe, symlinked, or Git-ignored evidence directories fall back to temporary storage for that run.
- Screenshots, images, GIFs, and videos without a repository path, externally visible URL, or managed evidence-directory path eligible for hosted upload are reported as `ask-user` warnings because the generated PR would otherwise show only a local file path.
...
- Earlier Review output remains evidence for the commit it inspected, and the PR body does not promote Review risk rationale into final-scope claims.
- PR body includes a `## Intent` section when user intent is available, a `## Basecamp` section when references are detected, the final-diff `## What Changed`, an optional `## Testing` section, and a compact `## Pipeline` section. Pipeline entries expose step status and outcome counts only; Review is rendered as completed without its earlier risk rationale.
- Basecamp card references are detected in sanitized intent and full commit messages from canonical `app.basecamp.com` URLs, legacy `3.basecamp.com` URLs, `BC#<card-id>`, or `Basecamp card <card-id>`. The regenerated `## Basecamp` section sits between Intent and What Changed, deduplicates by card ID in first-seen order, and prefers an intent URL over commit metadata. Bare IDs stay visible as unlinked entries and emit warning-level `no-op` PR findings instead of blocking PR creation.
