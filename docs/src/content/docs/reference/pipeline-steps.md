...
- The step records the exact tests and checks it exercised in a `tested` array, may include a short natural-language `testing_summary`, and includes an `artifacts` array for reviewer-visible evidence; `path` artifacts should point to the dedicated evidence directory or to repository-relative paths that will be pushed, `url` artifacts must be externally visible, and `content` artifacts should be short logs or command output shown directly in the PR.
- By default, evidence is stored in a managed temporary directory for the run, and GitHub visual evidence is uploaded to secret gists while the PR body is assembled. `test.evidence.store_in_repo: true` writes under `<test.evidence.dir>/<branch-slug>` inside the worktree so the push step can commit and publish evidence with the branch; unsafe directories, symlinked directories, and Git-ignored evidence directories fall back to temporary storage.
...
- Intent, Basecamp, `## What Changed`, risk, and testing sections are kept ahead of pipeline history. If unbounded testing evidence would displace a detected Basecamp section, Testing is omitted first; if the remaining essential sections or newest pipeline update are still too large, the PR step truncates at line or section boundaries and adds an explicit marker.
