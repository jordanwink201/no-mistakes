...
- The step records the exact tests and checks it exercised in a `tested` array, may include a short natural-language `testing_summary`, and includes an `artifacts` array for reviewer-visible evidence; `path` artifacts should be repository-relative paths that will be pushed, `url` artifacts must be externally visible, and `content` artifacts should be short logs or command output shown directly in the PR.
- By default, evidence is stored under `<test.evidence.dir>/<branch-slug>` inside the worktree, staged during push, and published with the branch. `test.evidence.store_in_repo: false`, unsafe directories, symlinked directories, and Git-ignored evidence directories fall back to temporary storage for that run.
- Screenshots, images, GIFs, and videos without a repository path or externally visible URL are reported as `ask-user` warnings because the generated PR would otherwise show only a local file path; GitHub PRs can still publish approved temporary visual evidence through secret gists when `test.evidence.upload_to_gist` is true.
